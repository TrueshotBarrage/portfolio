...
const PseudoLink = ({ id, text, url, clickedLink, setClickedLink }) => {
  React.useEffect(() => {
    if (clickedLink === id) {
      pseudoLinksActivator(id);
    }
  }, [clickedLink]);
  /**
   ...
   */
  const pseudoLinksActivator = function (id) {
    // Get the cursor out so appending to the end isn't malformed
    document.getElementById("cursor")?.remove();
    console.log("pseudoLinksActivator called");
    console.log(id);

    // const afterPLClicked = createRoot(
    //   document.getElementById("after-pseudo-link-clicked")
    // );
    // afterPLClicked.render(
    //   <CodeTextBox clickedLink={id} setClickedLink={setClickedLink} />
    // );
  };

  const setState = function () {
    if (clickedLink === null) {
      // console.log("clickedLink is null");
  ...
  return (
    <li id={id} className="pseudo-links" onClick={setState} key={id}>
      {text}
    </li>
  );
};

const runPseudoLinksGenerationString = async function (
  clickedLink,
  setClickedLink
) {
  const plContainer = createRoot(
    document.getElementById("pseudo-links-container")
  );

  const PseudoLinks = ({ clickedLink, setClickedLink }) => {
    return navLinks
      .filter((link) => link.id !== null)
      .map((link) => (
        <PseudoLink
          id={link.id}
          text={link.terminal_text}
          url={link.url}
  ...
  plContainer.render(
    <PseudoLinks clickedLink={clickedLink} setClickedLink={setClickedLink} />
  );
};
...
const CodeTextBox = ({ clickedLink, setClickedLink }) => {
  ...
  // const awaitPseudoLinkClick = async function () {
  //   if (clickedLink === null) {
  //     setTimeout(() => {
  //       awaitPseudoLinkClick();
  //     }, 100);
  //   } else {
  //     return clickedLink;
  //   }
  ...
  const goToLink = async function (link) {
    // window.location(link);
    // const navigate = useNavigate();
    // navigate(link);
  };

  const codeBoxContents = !clickedLink
    ? [
        {
        ...
        {
          text: `<ul id="pseudo-links-container"></ul>`,
          typewriter: false,
          delay: 0,
        },
        {
          text: runPseudoLinksGenerationString,
          args: [clickedLink, setClickedLink],
        },
        {
        ...
        onInit={(writer) => {
          ...
          for (let i = 0; i < codeBoxContents.length; i++) {
            ...
            if (typeof item.text === "string") {
              ...
              let returnedVal = writer.callFunction(() => {
